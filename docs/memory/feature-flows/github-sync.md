...
| Date | Changes |
|------|---------|
| 2026-04-19 | **Sync observability** (#389): dual `ahead_main`/`ahead_working` + `behind_main`/`behind_working` in `GET /api/git/status` (fixes P6). New `auto_sync_enabled` and `freeze_schedules_if_sync_failing` columns on `agent_git_config`. Full sync-health feature documented in [git-sync-health.md](git-sync-health.md). |
